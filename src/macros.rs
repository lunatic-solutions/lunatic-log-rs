...
macro_rules! event {
    ...
    (target: $target:expr, $lvl:expr, { $($fields:tt)* } )=> ({
        use $crate::Callsite as _;
        let mut callsite: $crate::Callsite = $crate::callsite2! {
            name: concat!(
                "event ",
                file!(),
                ":",
    ...
    (target: $target:expr, $lvl:expr, { $($fields:tt)* }, $($arg:tt)+ ) => (
        $crate::event!(
            target: $target,
            $lvl,
            { message = format!($($arg)+).into(), $($fields)* }
        )
    );
...
macro_rules! callsite2 {
    ...
    (
        ...
        // use $crate::{Callsite, Registration};
        ...
        // static REG: Registration = Registration::new($crate::Callsite::new(META));

        $crate::Callsite::new(metadata)
    }};
}
...
macro_rules! valueset {
    ...
    (@ { $(,)* $($out:expr),* }, $next:expr, $($rest:tt)+) => {
        $crate::valueset!(@ { ($next, Some(format!($($rest)+).into())), $($out),* }, $next, )
    };
    ...
    ($fields:expr, $($kvs:tt)+) => {
        {
            ...
            $fields.clone().value_set($crate::valueset!(
                @ { },

...
pub use lunatic_tracing_core::span::{Attributes, Id, Record};
...
impl Span {
    ...
    pub fn new(meta: Metadata, values: field::ValueSet) -> Span {
        dispatch::get_default(|dispatch| Self::new_with(meta, values, &dispatch))
    }

    #[inline]
    #[doc(hidden)]
    pub fn new_with(meta: Metadata, values: field::ValueSet, dispatch: &Dispatch) -> Span {
        let new_span = Attributes::new(meta.clone(), values);
    ...
    /// [field values]: super::field::ValueSet
    /// [`follows_from`]: super::Span::follows_from()
    pub fn new_root(meta: Metadata, values: field::ValueSet) -> Span {
        dispatch::get_default(|dispatch| Self::new_root_with(meta, values, &dispatch))
    }

    #[inline]
    #[doc(hidden)]
    pub fn new_root_with(meta: Metadata, values: field::ValueSet, dispatch: &Dispatch) -> Span {
        let new_span = Attributes::new_root(meta.clone(), values);
        Self::make_with(meta, new_span, dispatch)
    }
    ...
    pub fn child_of_with(
        ...
        let new_span = match parent.into() {
            Some(parent) => Attributes::child_of(parent, meta.clone(), values),
            None => Attributes::new_root(meta.clone(), values),
        };
        Self::make_with(meta, new_span, dispatch)
    }
    ...
    /// enabled.
    #[inline(always)]
    pub fn new_disabled(meta: Metadata) -> Span {
        Self {
            inner: None,
            meta: Some(meta),
        }
    ...
    fn make_with(meta: Metadata, new_span: Attributes, dispatch: &Dispatch) -> Span {
        let attrs = new_span;
    ...
    fn do_enter(&self) {
        if let Some(inner) = self.inner.as_ref() {
            inner.collector.enter(inner.id.clone());
        }
    ...
    fn do_exit(&self) {
        if let Some(inner) = self.inner.as_ref() {
            inner.collector.exit(inner.id.clone());
        }
    ...
    /// [`Metadata`]: super::Metadata
    pub fn record<Q: ?Sized>(&self, field: &Q, value: JsonValue) -> &Self
    where
        Q: field::AsField,
    {
        if let Some(meta) = &self.meta {
            if let Some(field) = field.as_field(meta) {
                self.record_all(meta.fields().clone().value_set(vec![(field, Some(value))]));
            }
        }
...
impl Inner {
    ...
    fn follows_from(&self, from: Id) {
        self.collector.record_follows_from(self.id.clone(), from)
    }
    ...
    fn record(&self, values: Record) {
        self.collector.record(self.id.clone(), values)
    }
...
impl Clone for Inner {
    fn clone(&self) -> Self {
        Inner {
            id: self.collector.clone_span(self.id.clone()),
...
mod test {
    ...
    fn test_record_backwards_compat() {
        Span::current().record("some-key", "some text".into());
        Span::current().record("some-key", false.into());
    }
}

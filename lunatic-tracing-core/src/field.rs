...
#[derive(Clone, Serialize, Deserialize)]
pub struct ValueSet {
    values: Vec<(Field, Option<JsonValue>)>,
    fields: FieldSet,
}
...
impl fmt::Debug for dyn Value {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        static FIELD: Field = Field {
            i: 0,
            fields: FieldSet::from_static(vec![], crate::identify_callsite!(0)),
        };
...
impl crate::sealed::Sealed for JsonValue {}
...
impl Value for JsonValue {
    fn record(&self, key: &Field, visitor: &mut dyn Visit) {
        ...
        // fn record_debug(&mut self, field: &Field, value: &dyn fmt::Debug);
        match self {
            JsonValue::Null => visitor.record_debug(key, self),
            JsonValue::Bool(b) => visitor.record_bool(key, *b),
            JsonValue::Number(n) => {
                if let Some(num) = n.as_u64() {
                    visitor.record_u64(key, num);
                } else if let Some(num) = n.as_i64() {
                    visitor.record_i64(key, num);
                } else if let Some(num) = n.as_f64() {
                    visitor.record_f64(key, num);
                } else {
                    unreachable!("number should have been repesentable as u64, i64, or f64");
                }
            }
            JsonValue::String(s) => visitor.record_str(key, s),
            JsonValue::Array(values) => visitor.record_debug(key, values),
            JsonValue::Object(obj) => visitor.record_debug(key, obj),
        }
    }
}
...
impl FieldSet {
    ...
    pub fn new(names: &'static [&'static str], callsite: callsite::Identifier) -> Self {
        Self {
            names: names.iter().map(|s| Cow::Borrowed(*s)).collect(),
            callsite,
        }
    }
    ...
    /// elements.
    #[doc(hidden)]
    pub fn value_set(self, values: Vec<(Field, Option<JsonValue>)>) -> ValueSet {
        ValueSet {
            fields: self,
            values,
        }
...
impl<'a> Iterator for Iter<'a> {
    ...
    fn next(&mut self) -> Option<Field> {
        ...
        Some(Field {
            i,
            fields: FieldSet {
                names: self.fields.names.clone(), // TODO: Remove clone
                callsite: self.fields.callsite(),
            },
        })
    }
